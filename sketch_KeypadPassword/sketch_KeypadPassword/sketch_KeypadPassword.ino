...
Password password = Password( "1903332088" ); //angak kode PIN 
...
Keypad ridwan = Keypad( makeKeymap(keys), PinBaris,   PinKolom, JmlBaris, JmlKolom ); 
void setup() { 
  ...
  delay(500);
  lcd.clear();
  ridwan.addEventListener(keypadEvent);
}
void loop() {
  pinMode(led, OUTPUT);
   if (a==0) { //state awal
  ...
  ridwan.getKey(); //membaca keypad
}
void keypadEvent(KeypadEvent eKey){ 
  switch (ridwan.getState()){
